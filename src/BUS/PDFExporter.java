...
document.add(tableKy);
    ...
    public static void HoaDonXuat(SalesInvoice hd){
...
Font thinFont = new Font(thinBaseFont, 11); // nhỏ hơn một chút và mỏng hơn
            ...
            // Tiêu đề
            Paragraph title = new Paragraph("HÓA ĐƠN XUẤT HÀNG", titleFont);
            title.setAlignment(Element.ALIGN_CENTER);
            title.setSpacingAfter(15f);
            document.add(title);
            ...
            // Thông tin hóa đơn
            SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
            document.add(new Paragraph("Mã hóa đơn: " + hd.getId(), normalFont));
            document.add(new Paragraph("Nhân viên: " +employeeDAO.getEmployeeById(hd.getEid()) , normalFont));
            document.add(new Paragraph("Khách hàng: " +  customerDAO.getCustomerbyId(hd.getEid()), normalFont));
            document.add(new Paragraph("Ngày lập: " + sdf.format(hd.getDate()), normalFont));
            document.add(new Paragraph(" ")); // khoảng trắng
            ...
            Map<String, Integer> Danhsach = phieuNhapBUS.getSoLuongTheoPhanLoai(hd.getId());
            int stt = 1;
            for (Map.Entry<String, Integer> entry : Danhsach.entrySet()) {
                String idPhanLoai = entry.getKey();
                int soLuong = entry.getValue();
