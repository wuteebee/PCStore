...
public class ThongKeKhachHang extends JPanel implements ActionListener, KeyListener, PropertyChangeListener {
    ...
    private ArrayList<ThongKeKhachHangDTO> list;
    ...
    public void updateTaskbar() {
        ...
        long totalOrders = list.stream().mapToLong(ThongKeKhachHangDTO::getSoluongphieu).sum();
        long totalMoney = list.stream().mapToLong(ThongKeKhachHangDTO::getTongtien).sum();
        listitem[0].setValue(String.valueOf(totalCustomers));
        listitem[1].setValue(String.valueOf(totalOrders));
    ...
    public void Fillter() throws ParseException {
        if (validateSelectDate()) {
            ...
            if (topN > 0) {
                list.sort(Comparator.comparingLong(ThongKeKhachHangDTO::getTongtien).reversed());
                if (list.size() > topN) {
